...
def _cache_vide():
    """``get_settings`` est mis en cache : sans purge, l'environnement posé ici
    n'atteindrait jamais l'objet lu par ``paths``."""
    get_settings.cache_clear()
    yield
    get_settings.cache_clear()


def test_runtime_root_env_var(monkeypatch, tmp_path):
...
def test_l_ancien_nom_ne_fait_rien(monkeypatch, tmp_path):
...
def test_un_chemin_absolu_ignore_la_racine(monkeypatch, tmp_path):
